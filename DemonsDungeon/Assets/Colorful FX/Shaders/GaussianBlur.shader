	...
	SubShader
	{
		...
		Pass
		{
			CGPROGRAM
				...
				fInput vert(appdata_img v)
				{
					...
					o.uv1 = float4(StereoScreenSpaceUVAdjust(v.texcoord.xy + d1, _MainTex_ST), StereoScreenSpaceUVAdjust(v.texcoord.xy - d1, _MainTex_ST));
					o.uv2 = float4(StereoScreenSpaceUVAdjust(v.texcoord.xy + d2, _MainTex_ST), StereoScreenSpaceUVAdjust(v.texcoord.xy - d2, _MainTex_ST));
					return o;
				}
		...
		Pass
		{
			CGPROGRAM
				...
				{
					half2 uv = i.uv;
					half4 oc = tex2D(_MainTex, StereoScreenSpaceUVAdjust(uv, _MainTex_ST));
					#if UNITY_UV_STARTS_AT_TOP
					if (_MainTex_TexelSize.y < 0)
						uv.y = 1.0 - uv.y;
					#endif
					half4 bc = tex2D(_Blurred, StereoScreenSpaceUVAdjust(uv, _Blurred_ST));
