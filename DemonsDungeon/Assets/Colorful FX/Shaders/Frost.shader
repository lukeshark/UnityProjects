...
Shader "Hidden/Colorful/Frost"
{
	...
	CGINCLUDE
		...
		{
			float2 uv = i.uv;
			half4 color = tex2D(_MainTex, StereoScreenSpaceUVAdjust(uv, _MainTex_ST));
			...
			half4 frosted = tex2D(_MainTex, StereoScreenSpaceUVAdjust(uv + float2(dx, dy) * _Scale, _MainTex_ST));
			return frosted;
		}
