...
function ExportSubstances(){
	...
	var progressBarString	: String	= "Starting Mass Texture Exportation";													// Set the variable for the progress bar text
	var currentPercent		: float		= 0.0;																					// Set the variable for the progress percent
	EditorUtility.DisplayProgressBar("Mass Texture Exporter", progressBarString, currentPercent);								// Start the progress bar at 0
	for (var i : int; i < substances.Length; i++){
		progressBarString	= "(Group " + (i + 1) + " of " + substances.Length + ") Starting " + substances[i].name;				// Set String
		if (i != 0)
			currentPercent		= (i * 1.0) / (substances.Length * 1.0);																		// Computer percent
		EditorUtility.DisplayProgressBar("Mass Texture Exporter", progressBarString, currentPercent);							// Update progress bar
		...
		for (var s : int; s < substanceMaterials.Length; s++){		
			...
			progressBarString	= "(Group " + (i + 1) + " of " + substances.Length + ") " + substances[i].name + " | " + substanceMaterial.name + " (" + (s + 1) + " of " + substanceMaterials.Length + ")";
			var groupPercent	= 0.0;
			if (i != 0)
				groupPercent		= (i * 1.0) / (substances.Length * 1.0);
			var materialPercent	= ((s * 1.0) / (substanceMaterials.Length * 1.0)) / (substances.Length * 1.0);
			currentPercent		= groupPercent + materialPercent;																// Computer percent
			EditorUtility.DisplayProgressBar("Mass Texture Exporter", progressBarString, currentPercent);						// Update progress bar

			ExportSubstance(substanceImporter, substanceMaterial);
		}
	}
	EditorUtility.DisplayProgressBar("Mass Texture Exporter", "Finishing Up...", 100);											// Update progress bar
...
function ExportSubstance(substanceImporter : SubstanceImporter, substanceMaterial : ProceduralMaterial){
	...
	if (overrideNameEnabled)
		previousPath					= "Assets/SFBayStudios/Exported Materials/" + previousGroupName + "/tex_" + overrideMaterialName + "/";
	...
	for (var materialTexture : ProceduralTexture in materialTextures) {
			...
			materialTexture.name == substanceMaterial.name + "_emissive")
		{
			...
			//Debug.Log("materialTexture.name = " + materialTexture.name);
			//Debug.Log("substanceMaterial.name + _normal = " + substanceMaterial.name + "_normal");
			//Debug.Log("useNormalFromPrevious: " + useNormalFromPrevious);
			//Debug.Log(previousPath + "" + materialTexture.name + ".tga Exists: " + System.IO.File.Exists(previousPath + materialTexture.name + ".tga"));
