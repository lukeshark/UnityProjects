...
private var saveChanges				: boolean			= true;

private var progressBarTitle		: String			= "Copy This To That";	// Used for progress bar
private var progressBarString		: String;									// Used for progress bar
private var progressPercent			: float				= 0.0;					// Used for progress bar
...
function Options(newIncludeMain : boolean, newIncludeEnv : boolean, newParseEnv : boolean, newSaveChanges : boolean){
...
function UpdateAllSources(materialID : int){
	for (var i : int; i < actions.Count; i++){
		actions[i].sourceMaterial	= materialID;
	}
}

function UpdateAllGroups(materialID : int, groupID : int){
	for (var i : int; i < actions.Count; i++){
		if (targets[actions[i].targetMaterial].groups[actions[i].targetGroup].name != "Env" && targets[actions[i].targetMaterial].groups[actions[i].targetGroup].name != "Main")
			actions[i].sourceGroup		= groupID;
	}
}
...
function SingleAction(actionID : int){
	...
	progressBarString	= "This may take a moment...";
	EditorUtility.DisplayProgressBar("Copy This To That: Rebuilding Textures", progressBarString, 0.5);
...
function CopyThisToThat(actionID : int){
	...
	progressBarString	= "Copying " + sources[sourceMaterialID].name + " " + sourceName + " to " + targets[targetMaterialID].name + " " + targetName;
	EditorUtility.DisplayProgressBar(progressBarTitle, progressBarString, progressPercent);
	...
	//	for (var snp : int; snp < sourceNameParts.Length; snp++){
	//		print ("sourceNameParts[" + snp + "]: " + sourceNameParts[snp]);
	//	}
		...
		//Debug.Log (sourceName + " | " + sourceNameParts[0]);

		if (sourceName == sourceNameParts[0])
		{
			...
			for (var v : int; v < targets[targetMaterialID].groups[targetGroupID].values.Count; v++){
				...
				{
					//print ("targetNameParts[0]: " + targetNameParts[0]);
					//Debug.Log ("Target/Source Suffix: " + targetSuffix + " / " + sourceSuffix);
					canCopy	= true;
				}
				...
				{
					//Debug.Log (targets[targetMaterialID].groups[targetGroupID].values[v].name);
					//Debug.Log (sources[sourceMaterialID].groups[sourceGroupID].values[sv].name);
					canCopy = true;
				}
				...
				if (canCopy)
				{
					...
					progressBarString	= "Copying Source " + sources[sourceMaterialID].groups[sourceGroupID].values[sv].name + " to " + targets[targetMaterialID].groups[targetGroupID].values[v].name;
					EditorUtility.DisplayProgressBar(progressBarTitle, progressBarString, progressPercent);
...
function GetOption(optionName : String) : boolean {
	if (optionName	== "saveChanges")
		return	saveChanges;
	if (optionName	== "parseEnv")
		return	parseEnv;
	if (optionName	== "includeMain")
		return	includeMain;
	if (optionName	== "includeEnv")
		return	includeEnv;
}

function AllActions(){
	progressBarString	= "Copying All Actions: Go!";
	EditorUtility.DisplayProgressBar(progressBarTitle, progressBarString, progressPercent);
	for (var a : int; a < actions.Count; a++){
		progressPercent		= (a * 1.0) / (actions.Count * 1.0);
		CopyThisToThat(a);
	}
	progressBarString	= "This may take a moment...";
	EditorUtility.DisplayProgressBar("Copy This To That: Rebuilding Textures", progressBarString, 0.5);
...
function GroupActions(targetID : int){
	progressBarString	= "Copying Actions for Group #" + targetID;
	EditorUtility.DisplayProgressBar(progressBarTitle, progressBarString, progressPercent);
	for (var a : int; a < actions.Count; a++){
		if (actions[a].targetMaterial == targetID)
		{
			progressPercent		= (a * 1.0) / (actions.Count * 1.0);
			CopyThisToThat(a);
		}
	...
	progressBarString	= "This may take a moment...";
	EditorUtility.DisplayProgressBar("Copy This To That: Rebuilding Textures", progressBarString, 0.5);
	StartRebuildingTexture(targetID);
	EditorUtility.ClearProgressBar();	
}
...
function StartRebuildingTexture(targetID : int){
	...
	if (!targets[targetID].material.isProcessing)
	{
		...
		if (saveChanges)
		{
			Debug.Log ("Saved Substance");
			substanceImporter.SaveAndReimport();
		}
		targets[targetID].material.RebuildTextures();
		//AssetDatabase.WriteImportSettingsIfDirty(AssetDatabase.GetAssetPath(targets[targetID].material));
	}
}