...
public class SFB_CopyThisToThatEditor extends Editor
{
	...
	function OnInspectorGUI()
    {
		...
		if (showOptionsBox)
		{
			...
			myScript.Options(includeMain,includeEnv,parseEnv,saveChanges);
		...
		if (myScript.sources.Count > 0 && myScript.targets.Count > 0)
		{
			...
			if (showCopyBox)
			{
				...
				GUILayout.Label ("-------------------- Update All Sources --------------------");
				EditorGUILayout.BeginHorizontal ();
					materialAll	= EditorGUILayout.IntPopup(materialAll, myScript.SourceNames(), myScript.SourceIDs());
					GUI.color 			= Color.white;
					if (GUILayout.Button ("Update ALL Sources", GUILayout.Height(15)))
					{
						myScript.UpdateAllSources(materialAll);
						groupAll		= 1;
					}
					GUI.color 			= Color.white;
					groupAll 	= EditorGUILayout.IntPopup(groupAll, myScript.SourceGroupNames(materialAll), myScript.SourceGroupIDs(materialAll));
					GUI.color 			= Color.white;
					if (GUILayout.Button ("Update ALL Groups", GUILayout.Height(15)))
					{
						myScript.UpdateAllGroups(materialAll, groupAll);
					}
					GUI.color 			= Color.white;
				EditorGUILayout.EndHorizontal ();
				...
				for (var t : int; t < targetsCount; t++){
							...
							for (var sm : int; sm < myScript.GetCount("actions"); sm++){
								if (myScript.actions[sm].targetMaterial == t)
								{
									...
									if (myScript.GetCount("sources") > 1)
									{
										...
										{
											myScript.actions[sm].sourceGroup	= myScript.FirstNonGlobalGroup(myScript.actions[sm].targetMaterial, myScript.actions[sm].targetGroup, newID);
										}
					...
					GUI.color 			= Color.green;
					if (GUILayout.Button ("Copy Actions: " + myScript.targets[t].name, GUILayout.Height(15)))
					{
						myScript.GroupActions(t);
					}
					GUI.color 			= Color.white;
				}		
			}
