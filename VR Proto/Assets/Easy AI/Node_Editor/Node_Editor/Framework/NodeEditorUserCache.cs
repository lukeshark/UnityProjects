...
namespace NodeEditorFramework
{
	public class NodeEditorUserCache
	{
		...
		void dummy_code(){
			// this is only for buil purpose => for not to show warning in console
			useCache = false;
			cachePath ="";
			useCache = true;
		}
		public NodeEditorUserCache (MainNodeCanvas loadedCanvas)
		{
			useCache = false;
		...
		#if UNITY_EDITOR
		public NodeEditorUserCache (string CachePath, MainNodeCanvas loadedCanvas)
		{
			useCache = true;
			cachePath = CachePath;
			SetCanvas (loadedCanvas);
		...
		public void SaveCache () 
		{
			...
			if (!useCache)
				return;
			if (!nodeCanvas || nodeCanvas.GetType () == typeof(MainNodeCanvas))
				return;
		...
		private void CheckCurrentCache () 
		{
			...
			else if (UnityEditor.AssetDatabase.LoadAssetAtPath<MainNodeCanvas> (lastSessionPath) == null)
				SaveCache ();
			#endif
		}
		...
		public void SetCanvas (MainNodeCanvas canvas)
		{
		...
		{
			nodeCanvas = MainNodeCanvas.CreateCanvas (canvasType);
		...
		{
			MainNodeCanvas canvas = NodeCanvasManager.ConvertCanvasType (nodeCanvas, newType);
