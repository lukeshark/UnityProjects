		...
		Pass
		{
			...
			CGPROGRAM
				...
				half4 frag(v2f_img i) : SV_Target
				{
					...
					// Top
					temp = tex2D(_MainTex, StereoScreenSpaceUVAdjust(i.uv + half2(-_PSize.x, -_PSize.y), _MainTex_ST));
					color += temp * _KernelT.x;

					temp = tex2D(_MainTex, StereoScreenSpaceUVAdjust(i.uv + half2(0.0, -_PSize.y), _MainTex_ST));
					...
					// Middle
					temp = tex2D(_MainTex, StereoScreenSpaceUVAdjust(i.uv + half2(-_PSize.x, 0.0), _MainTex_ST));
					color += temp * _KernelM.x;

					origin = tex2D(_MainTex, StereoScreenSpaceUVAdjust(i.uv, _MainTex_ST));
