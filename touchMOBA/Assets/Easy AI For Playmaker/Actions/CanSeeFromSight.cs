...
namespace HutongGames.PlayMaker.Actions
{
	...
	[Tooltip ("Can See Object From Sight.")]
	public class CanSeeFromSight : FsmStateAction
	{
		[Tooltip ("The object From sight")]
		public FsmOwnerDefault goFromSight;
		...
		[Tooltip ("This event will fire when the target was seen \r")]
		...
		{
			if (goFromSight != null)
				go = Fsm.GetOwnerDefaultTarget (goFromSight);
		...
		{
			if (Owner != null) {
				if (goFromSight.GameObject.Value != null)
					DrawLineOfSight (goFromSight.GameObject.Value.transform, offset.Value, fieldOfViewAngle.Value, viewDistance.Value, usePhysics2D.Value);
				else
