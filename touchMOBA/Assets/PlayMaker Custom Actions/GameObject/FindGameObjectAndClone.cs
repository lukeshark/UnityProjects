...
namespace HutongGames.PlayMaker.Actions
{
	...
	[Tooltip("Work exactly like FindGameObject, but this action is trying to find gameobject (Clone) if nothing is return.")]
	public class FindGameObjectAndClone : FsmStateAction
	{
		...
		public override void OnEnter()
		{
			...
			if (store.Value = null)
			{
				//Result = null so try to find objectName + (Clone)
				objectName = objectName + "(Clone)";
				FindGameObject();
			}
			if (store.Value == null)
				LogWarning ("No Object " + objectName + " found!");
